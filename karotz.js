 ...
 * @param  {bool}     permanent If true, the module authentificate the app every 14 min. (An installId is only valid 15 min)
 */
	...
	request(url, function (error, response, body) {
		...
		if(typeof body == 'undefined') {
			...
			parseString(body, function (err, result) {
			    ...
			    if(!err)
			    {
			    	if(typeof result.VoosMsg.response != "undefined"
			    		&& result.VoosMsg.response[0].code[0] == "OK"
						)
			    	{
			    		msg = "OK"; //Action done
